...
KEYSYM = {
    ...
    # browser-screencast targets Linux/Windows/BSD; browser Meta/Cmd maps to
    # the remote Super/Windows key rather than macOS Command.
...
class VNCBridge:
    ...
    def send_key(self, down, keysym):
        ...
        # macscreencast used a macOS HID nudge here for screensharingd. Keep it
        # only on Darwin; Linux/Windows VNC servers do not need it.
        if down and sys.platform == "darwin":
            try:
                import Quartz as _Q
    ...
    def send_clipboard(self, text):
        ...
        # paste keys to fire before the clipboard is set.
        ...
        if type_chars and text:
            for ch in text:
                if ch == "\n":
                    ...
                    continue
                self.send_key(True, ks)
                self.send_key(False, ks)
                time.sleep(0.005)
            return True
        if text:
            self.send_clipboard(text)
        if paste:
            for code, down in (("ControlLeft", True), ("KeyV", True),
                               ("KeyV", False), ("ControlLeft", False)):
                self.send_key_event(down, code, "v" if code == "KeyV" else "Control")
        return True
    ...
    def _pbpaste_poll(self):
        """Poll the local clipboard for browser-side clipboard sync."""
