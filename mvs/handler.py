...
from mvs.codec import (CODEC_JPEG, CODEC_H264, CODEC_H265, CODEC_AV1, CODEC_VP9,
                        _select_codec, _hdr, normalize_client_caps,
                        probe_server_codecs, select_codec)
...
async def client_session(ws, cfg, bridge):
    ...
    # Wait for capture backend to be ready.
    for _ in range(30):
        W, H = bridge.dimensions
        if W and H: break
        await asyncio.sleep(0.2)
    else:
        log.warning("capture backend not ready"); return
    ...
    _codec_by_name = {"h264": CODEC_H264, "h265": CODEC_H265,
                      "hevc": CODEC_H265, "av1": CODEC_AV1,
                      "vp9": CODEC_VP9, "jpeg": CODEC_JPEG}
    target_codec = _codec_by_name.get(cfg.codec, CODEC_H264)
    ...
    def _upgrade_encoder(tw: int = 0, th: int = 0, explicit: bool = False):
        ...
        old = encoder
        # Cascade: try target_codec first, then progressively safer fallbacks.
        ...
        if explicit and target_codec != CODEC_JPEG:
            ...
            fallbacks = [target_codec, CODEC_AV1, CODEC_H265, CODEC_VP9, CODEC_H264]
        new_enc = None
        for codec in fallbacks:
            ...
            e.close()
        if new_enc is None:
            _codec_labels = {CODEC_H264: "H.264", CODEC_H265: "H.265",
            ...
            log.info("Encoder %s %dx%d for %s",
                     {CODEC_H264:"h264",CODEC_H265:"h265",CODEC_AV1:"av1",CODEC_VP9:"vp9"}.get(new_enc.actual_codec,"?"),
            ...
            async for raw in ws:
                ...
                try:
                    ...
                    if t == "reset":
                            ...
                            if client_matrix:
                                ...
                                log.info("codec negotiation: %s server=%s client=%s",
                                         {CODEC_H264:"h264",CODEC_H265:"h265",
                                          CODEC_AV1:"av1",CODEC_VP9:"vp9",
                                          CODEC_JPEG:"jpeg"}.get(target_codec, "?"),
                        ...
                        if not (hasattr(bridge, "send_scroll") and bridge.send_scroll(dx, dy, x, y)):
                            evts = []
                        ...
                        down = t == "kd"
                        sent = False
                        if hasattr(bridge, "send_key_event"):
                            sent = bool(bridge.send_key_event(down, code, k))
                        if not sent:
                        ...
                        text = ev.get("text","")
                        if t == "paste":
                            remote_rev = ev.get("mac_rev")
                            if (remote_rev is not None
                                    and remote_rev != bridge.server_clipboard_seq):
                                continue
                        if hasattr(bridge, "paste_text"):
                            res = bridge.paste_text(
                                text, paste=(t == "paste"), type_chars=bool(ev.get("type_chars")))
    ...
    async def frame_sender():
        ...
        try:
            ...
            pass
        # Send current remote clipboard immediately on connect so side menu is populated.
        ...
        try:
            while True:
                ...
                if now - _t_diag >= 5.0:
                    ...
                    _cap_n = bridge._fbu_count
                    _cap_fps = (_cap_n - _last_capture_fbu) / dt; _last_capture_fbu = _cap_n
                    _fb_age = int(time.time() * 1000) - bridge._fb_ms
                    await ws.send(json.dumps({"t": "stale", "ms": _fb_age}))
                    log.info("DIAG: sent=%d/%.1fs=%.1ffps drop_wb=%d nosend=%d ctrl_fps=%.1f ctrl_br=%dk cap=%.1ffps fb_age=%dms drain=%s",
                             _n_diag, dt, _n_diag/dt, _n_drop, _n_nosend, ctrl.fps, ctrl.bitrate // 1000, _cap_fps, _fb_age, ctrl.draining)
                ...
                # Video codecs (H.264/H.265/AV1/VP9): never drop here. The encoder's
                # bitrate target should self-correct; dropping an encoded P-frame
                # after encode() has run desynchronises encoder ↔ decoder reference
                # frames and produces block-echo corruption. Write-buffer monitoring
                # handles genuine network congestion independently.
...
def make_ws_handler(cfg, bridge):
    async def handler(ws):
        ...
        if not _check_token(ws.request.path if hasattr(ws, 'request') else "/", cfg.password):
            ...
            return
        if path == "/audio":
            if getattr(cfg, "no_audio", False):
                await ws.close(1000, "audio disabled")
            else:
