#!/usr/bin/env python3
"""ci_smoke.py — non-interactive CI smoke test for browser-screencast.

Tests:
  1. Video: connect WebSocket, send caps, assert >= MIN_FRAMES in 8s
  2. Audio: connect audio WebSocket, assert packets if audio is configured
...
async def main():
    print(f"browser-screencast CI smoke test — {HOST}:{PORT}")
    print()
    _cpu.start()
    try:
        await test_video()
